...
from excel_reader import clean_val

class FileInspector(QMainWindow):
    """Main GUI window for Portfolio Validator."""
    ...
    def select_project_folder(self):
        ...
        if not folder:
            return
        excel_files = [f for f in glob.glob(os.path.join(folder, "*.xlsx"))
    ...
    def load_data(self):
        """Read Excel and populate status cards."""
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    excel_info = {
                        "Criteria": clean_val(str(row[1]).split(".", 1)[1] if "." in str(row[1]) else "---"),
                        "Type": clean_val(row[2]),
                        "Context": clean_val(row[3]),
                        "Omschr": clean_val(row[4]),
                        "Niveau": clean_val(row[5]),
                        "Rel": clean_val(row[6]),
                        "Auth": clean_val(row[7]),
                        "Act": clean_val(row[8]),
                        "Datum": clean_val(row[9]),
                    }

                    folder_path = os.path.join(self.evidence_folder, eid)
                    found_files = [f for f in os.listdir(folder_path) if not f.startswith(".")] if os.path.isdir(folder_path) else []

                    has_excel_entry = excel_info["Type"] != "---" or excel_info["Context"] != "---"
                    has_files = bool(found_files)

                    if has_excel_entry and has_files:
                        if any(excel_info[k] == "---" or excel_info[k].upper() == "NEE" for k in ["Niveau", "Rel", "Auth", "Act", "Datum"]):
                            status = "INCOMPLETE"
                        else:
                            status = "GOOD"
    ...
    def display_details(self, card):
        ...
        todos = []
        def is_empty(v): return v == "---"
        def is_no(v): return v.upper() == "NEE"

        if not card.files: todos.append("❌ Geen bestanden aanwezig")
        if is_empty(d["Niveau"]): todos.append("⚠️ Niveau niet ingevuld")
        if is_empty(d["Rel"]): todos.append("⚠️ Relevant niet ingevuld")
        elif is_no(d["Rel"]): todos.append("❌ Bewijs is als niet relevant gemarkeerd")
        if is_empty(d["Auth"]): todos.append("⚠️ Authentiek niet ingevuld")
        elif is_no(d["Auth"]): todos.append("❌ Bewijs is als niet authentiek gemarkeerd")
        if is_empty(d["Act"]): todos.append("⚠️ Actueel niet ingevuld")
        elif is_no(d["Act"]): todos.append("❌ Bewijs is als niet actueel gemarkeerd")
        if is_empty(d["Datum"]): todos.append("⚠️ Datum niet ingevuld")

        todo_html = "<ul>" + "".join(f"<li>{t}</li>" for t in todos) + "</ul>" if todos else "<i>Bewijs is compleet 🎉</i>"
